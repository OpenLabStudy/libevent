...
protected:
    ...
    void SetUp() override {
        ...
        // 소켓 파일 생성 대기
        ASSERT_TRUE(waitForFileExists(UDS_COMMAND_PATH, 2000)) << "UDS not ready";
        // (선택) 러닝 여부 점검
...
TEST_F(UdsSvrRunTest, ConnectDisconnect) {
    int fd1 = connectUds(UDS_COMMAND_PATH);
    ASSERT_GE(fd1, 0) << strerror(errno);
    ::close(fd1);

    // 재접속 가능해야 함
    int fd2 = connectUds(UDS_COMMAND_PATH);
...
TEST_F(UdsSvrRunTest, KeepAliveCommandTest) {
    int iFd = connectUds(UDS_COMMAND_PATH);
...
TEST_F(UdsSvrRunTest, QuitLikeClose) {
    ...
    int iFd2 = connectUds(UDS_COMMAND_PATH);
    ASSERT_GE(iFd2, 0);
    ::close(iFd2);
}
